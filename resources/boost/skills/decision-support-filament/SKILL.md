...
- **Gating:** a guide's required permissions live at `extra_attributes.permissions` (engine column) and a match mode at `extra_attributes.permissions_mode` — `any` (OR) or `all` (AND), defaulting to config `permissions.mode`. The picker is a searchable multi-select driven by `permissions.options` (array, or `fn (?Guide $guide): array` per guide); with no catalog the field is a warning callout (existing permissions stay removable), and the mode shows when a catalog exists or the guide already has permissions. The "Required permissions" + "Permission match" fields are authoritative; each draft version has an "Edit metadata" action for its working copy (seeded onto the guide at publish). The package enforces nothing — read both `permissions` and `permissions_mode` in your host `Guide` policy.
- **Required questions:** a free (text/date/number) question can be toggled **Required** in the editor (`config.required`). The runner marks the prompt with a red asterisk and validates on submit (blank → inline error, no advance; Submit stays enabled); the engine re-suspends on blank as the backstop.
- **i18n (content):** set `locales`/`fallback_locale`. The editor adds a translation input per locale beside translatable fields (prompt, verdict, text, and each node's display label → `config.label_i18n`), stored in the node's `*_i18n` maps; blank inputs are dropped. The runner passes `app()->getLocale()` + `fallback_locale` to the engine, which resolves locale → fallback → base.
- **i18n (diagram):** the runner/editor Mermaid diagram renders node text in the panel locale (`getMermaidSourceProperty` passes the locale + fallback to the engine renderer). Give a fact/decision node a label + `label_i18n` so the diagram shows readable text instead of the raw key. **Edges** take a label + per-locale inputs too (stored on `guide_edges.label`/`label_i18n`), overriding the derived condition text. Node-type and input-type dropdown labels are translatable too (`editor.node_type.*`, `editor.input_type.*`), falling back to the raw key for custom node types.
- **i18n (validation panel):** the editor's live validation issues render via `validation.{code}` translations (en/da) filled with the engine's structured `ValidationError::$params`, falling back to the engine's English message for any unmapped code.
- **i18n (UI chrome):** all package UI strings use a `decision-support-filament` translation namespace. **`en` and `da` ship bundled** — set the panel locale to `da` for Danish chrome with no setup. To adjust wording or add a language, `php artisan vendor:publish --tag=decision-support-filament-translations` (publishes both `en` and `da`), then edit in place or copy a folder to a new locale.
